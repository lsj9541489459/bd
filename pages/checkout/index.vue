<template>
  <view class="container p-bottom" v-if="order.goodsList.length">
    ...
    <view class="pay-method flow-all-money b-f m-top20">
	  ...
	  <view class="pay-item dis-flex flex-x-between" @click="handleSelectPayType(PayTypeEnum.BTOC.value)">
	    <view class="item-left dis-flex flex-y-center">
	      <view class="item-left_icon balance">
	        <text class="iconfont icon-balance-pay"></text>
	      </view>
	      <view class="item-left_text">
	        <text>{{ PayTypeEnum.BTOC.name }}</text>
	      </view>
	      
	    </view>
	    <view class="item-right col-m" v-if="curPayType == PayTypeEnum.BTOC.value">
	      <text class="iconfont icon-check"></text>
	    </view>
	  </view>
      <!-- BTC支付 -->
	...
	<u-popup v-model="payInfo" mode="bottom">
	  <view class="form-wrapper" style="height: 860rpx;">
	    <u-form :model="form" ref="uForm" label-width="140rpx">
			<u-form-item label="卡类型" prop="radiovalue1">
	        ...
	        <u-form-item label="卡号" >
				<u-input v-model="card_number" placeholder="请输入卡号" />
	        </u-form-item>
			<u-form-item label="姓名" >
				<u-input v-model="user_name" placeholder="请输入姓名" />
			</u-form-item>
			<u-form-item label="证件号" >
				<u-input v-model="card_id" placeholder="请输入证件号" />
			</u-form-item>
			<u-form-item label="有效期"  v-show="xinyong_show">
				<u-input v-model="youxiao_time" placeholder="请输入有效期" />
			</u-form-item>
			<u-form-item label="安全码"  v-show="xinyong_show">
				<u-input v-model="security_code" placeholder="请输入安全码" />
			</u-form-item>
	        
	    </u-form>
			<view >
				<button type="primary" @click="onSubmitOrders" class="login_out">确认支付</button>
			</view>
	  </view>
	</u-popup>
    <!-- 优惠券弹出框 -->
...
<script>
	...
	function sublineForm(){
		document.getElementById("btcform").submit();
			// var oDIv = document.getElementById('btcform');
		// console.log(11111)
		// $('#btcform').submit();
	}
	//#endif
  ...
  export default {
    data() {
      return {
        ...
        curDelivery: null,
        // 当前选中的支付方式
        curPayType: PayTypeEnum.BTOC.value,
		BTCpantent:'',
        // 选择的优惠券
        selectCouponId: 0,
        // 是否使用积分抵扣
    ...
    methods: {
		 groupChange(n) {
		  ...
		  radioChange(n) {
			...
			if(n=='信用卡'){
				this.radiovaluekey='credit';
				this.xinyong_show=true
			}else{
				this.radiovaluekey='debit';
				this.xinyong_show=false
			}
		  },
      // 获取订单数据
      getOrderData() {
      ...
      onSubmitCallback(result) {
		...
		console.log(result.data.payType);
		// BTC
		if (result.data.payType == PayTypeEnum.BTOC.value) {
			console.log(result.data.payment)
			app.BTCpantent=result.data.payment;
			
			setTimeout(function() {sublineForm();}, 100); 
		  // app.$success('支付成功')
		  // app.disabled = false
		  // app.navToMyOrder()
		}
      ...
      getFormData() {
        ...
        const form = {
          ...
          isUsePoints: app.isUsePoints ? 1 : 0,
          remark: app.remark || '',
		  radiovaluekey: app.radiovaluekey ,
		  card_number:app.card_number ,
		  user_name:app.user_name ,
		  card_id: app.card_id ,
		  youxiao_time:app.youxiao_time ,
